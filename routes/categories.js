...
router.get('/', async (req, res) => {
  try {
    ...
    if (req.user.role === 'SUPERUSER') {
      ...
      const query = 'SELECT * FROM categories WHERE commerce_id = $1 ORDER BY id ASC';
...
router.post('/', async (req, res) => {
  try {
    ...
    const query = `
      INSERT INTO categories (commerce_id, name)
      VALUES ($1, $2)
      RETURNING *
    `;
    const values = [finalCommerceId, name];
    const result = await pool.query(query, values);
  ...
  try {
    ...
    if (req.user.role === 'SUPERUSER') {
      ...
      const query = `
        ...
        SET name = $1,
            updated_at = CURRENT_TIMESTAMP
        WHERE id = $2 AND commerce_id = $3
        RETURNING *
      `;
      const values = [name, id, commerceId];
      const result = await pool.query(query, values);
