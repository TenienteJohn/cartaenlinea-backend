...
router.post('/', authMiddleware, async (req, res) => {
  try {
    const { product_id, name, required, multiple, max_selections } = req.body;
    ...
    `;
    const productResult = await pool.query(productQuery, [product_id, req.user.commerceId]);
    ...
    const values = [product_id, name, required, multiple, max_selections];
...
router.post('/:optionId/items', authMiddleware, async (req, res) => {
  try {
    ...
    const values = [optionId, name, price_addition || 0, available !== false, image_url || null];
    const result = await pool.query(insertQuery, values);
